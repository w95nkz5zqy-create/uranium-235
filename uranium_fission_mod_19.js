...
elements.uranium_235 = {
    ...
    tick: function(pixel) {
        // Check all adjacent pixels for neutrons
            ...
            for (var dy = -1; dy <= 1; dy++) {
                ...
                if (!isEmpty(nx, ny, true)) {
                    ...
                    if (nearPixel && nearPixel.element === "neutron") {
                        ...
                        // Spawn 3 new neutrons immediately
                        for (var i = 0; i < 3; i++) {
                            var angle = Math.random() * Math.PI * 2;
                            var speed = 3 + Math.random() * 2;
                            var nnx = Math.round(pixel.x + Math.cos(angle) * speed);
                            var nny = Math.round(pixel.y + Math.sin(angle) * speed);
                            
                            if (isEmpty(nnx, nny)) {
                                createPixel("neutron", nnx, nny);
                            }
                        }
                        ...
                        // Occasionally spawn fission products
                        if (Math.random() < 0.3) {
                        ...
                        return; // Only handle one neutron per tick
                    }
                }
            }
        }
