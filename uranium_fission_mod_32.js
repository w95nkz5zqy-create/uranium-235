...
// Moderator - Light grey, lets particles through, slows neutrons AND protons once
elements.moderator = {
    color: ["#d3d3d3", "#c8c8c8", "#dedede", "#c0c0c0"],
    behavior: behaviors.SUPPORTPOWDER,  // Like glass - lets particles pass through!
    tick: function(pixel) {
        // Check for neutrons and protons passing through
        for (var dx = -1; dx <= 1; dx++) {
            for (var dy = -1; dy <= 1; dy++) {
                if (dx === 0 && dy === 0) continue;
                var nx = pixel.x + dx;
